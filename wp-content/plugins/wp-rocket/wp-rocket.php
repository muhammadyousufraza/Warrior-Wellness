...
// Ensure that the script is being executed within the WordPress environment
defined( 'ABSPATH' ) || exit;

// Delete specific transients related to WP Rocket
delete_transient( 'rocket_check_key_errors' ); // Remove any errors related to key checks
delete_transient( 'wp_rocket_no_licence' ); // Remove any transient indicating no license

// Define consumer data for WP Rocket
$consumer_data = [
    'consumer_key'   => '********', // Placeholder for the consumer key
    'consumer_email' => 'activated@wp-rocket.me', // Email associated with the consumer
    'secret_key'     => hash( 'crc32', 'activated@wp-rocket.me' ), // Generate a hash for the secret key
];

// Update WP Rocket settings with the new consumer data
update_option( 'wp_rocket_settings', array_merge( get_option( 'wp_rocket_settings', [] ), $consumer_data ) );

// Filter HTTP requests to mock responses for specific WP Rocket endpoints
add_filter( 'pre_http_request', function( $pre, $parsed_args, $url ) {
    // Check if the request URL matches the validation endpoint
    if ( strpos( $url, 'https://wp-rocket.me/valid_key.php' ) !== false ) {
        // Return a mocked successful response with consumer data
        return [
    ...
    // Check if the request URL matches the user stats endpoint
    elseif ( strpos( $url, 'https://wp-rocket.me/stat/1.0/wp-rocket/user.php' ) !== false ) {
        // Return a mocked response with license account and expiration details
        return [
            'response' => [ 'code' => 200, 'message' => 'OK' ],
            'body'     => json_encode( [
                'licence_account'    => '-1',
    ...
    // Return the original request if the URL does not match any of the above
    return $pre;
}, 10, 3 );
